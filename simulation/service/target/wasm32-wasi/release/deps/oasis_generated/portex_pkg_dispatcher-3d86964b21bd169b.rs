...
fn _oasis_dispatcher() {
    ...
    let output: std::result::Result<Vec<u8>, ()> = match payload {
        RpcPayload::extract_key_simulation((id,)) => Ok(oasis_std::reexports::serde_cbor::to_vec(
            &service.extract_key_simulation(&ctx, id),
        )
        .unwrap()),
    };
    match output {
        Ok(output) => oasis_std::backend::ret(&output),
        Err(err_output) => unreachable!("No RPC function returns Err"),
    }
}
