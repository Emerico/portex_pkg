...
//! code borrowed from https://docs.rs/crate/threshold_crypto/0.3.2/source/src/poly.rs 

//use crate::hash_g1;
use crate::cmp_projective;
...
//use crate::rand05::RngExt;
//mod rand04;
//use crate::rand04_compat::RngExt;
...
use crate::{Fr, G2Affine, G2};

//mod libs;
//pub use crate::libs::into_fr::IntoFr;

//use crate::{Fr, G1Affine, G1, G2Affine, G2};
...
impl Zeroize for Poly {
    fn zeroize(&mut self) {
	    //iterate over the fr and then clear it
...
impl Poly {
    ...
    pub fn commitment(&self) -> Commitment {
        ...
        //let to_g1 = |c: &Fr| hash_g1("bob".to_string()).into_affine().mul(*c);

        let to_g2 = |c: &Fr| G2Affine::one().mul(*c);
...
/// [ds] A commitment to a univariate polynomial.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct Commitment {
    /// The coefficients of the polynomial.
    #[serde(with = "super::serde_impl::projective_vec")]
    pub(super) coeff: Vec<G2>,
}
...
impl Commitment {
    ...
    pub fn evaluate<T: IntoFr>(&self, i: T) -> G2 {
        let mut result = match self.coeff.last() {
            None => return G2::zero(),
            Some(c) => *c,
        };
...
impl BivarPoly {
    ...
    pub fn commitment(&self) -> BivarCommitment {
       ...
       // let to_pub = |c: &Fr| hash_g1("bob".to_string()).into_affine().mul(*c);
        
        let to_pub = |c: &Fr| G2Affine::one().mul(*c);
...
impl BivarCommitment {
    ...
    pub fn evaluate<T: IntoFr>(&self, x: T, y: T) -> G2 {
        let x_pow = self.powers(x);
        let y_pow = self.powers(y);
        // TODO: Can we save a few multiplication steps here due to the symmetry?
    ...
    pub fn row<T: IntoFr>(&self, x: T) -> Commitment {
        let x_pow = self.powers(x);
        let coeff: Vec<G2> = (0..=self.degree)
            .map(|i| {
                let mut result = G2::zero();
                for (j, x_pow_j) in x_pow.iter().enumerate() {
                    let index = coeff_pos(i, j).expect("polynomial degree too high");
